...
<Note>Only if you set the `passIdToUnderlyingLLM` to `true`, the `endUser.id` will be passed to the underlying provider. Otherwise none of the information will be passed to the provider.</Note>
```typescript
{
  endUser?: {
    id: string
